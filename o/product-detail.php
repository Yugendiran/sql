...
if(isset($_GET['pid'])){
    ...
    if($product_count < 1){
        ...
        while($row = mysqli_fetch_assoc($select_product_result)){
            ...
            $db_products_views = $row['products_views'];
        }
    }

    // $insert_insight_query = "INSERT INTO insight(insight_pid, insight_dt) VALUES('$pid', '$current_date')";
    // $insert_insight_result = mysqli_query($connection, $insert_insight_query);

    // $db_products_views += 1;

    // $update_inght_count_query = "UPDATE products SET products_views = '$db_products_views' WHERE product_id = '$pid'";
    // $update_inght_count_result = mysqli_multi_query($connection, $update_inght_count_query);
}else{
    header("location: index.php");
}
?>
